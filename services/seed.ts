...
import { saveSession } from './firebaseService';
import { saveMasterList } from './firebaseService';
...
export const seedDatabase = async () => {
  ...
  // Save initial master list
  await saveMasterList(INITIAL_MASTERS);
  
  // Save initial sessions
  // Use Promise.all to save them in parallel
  await Promise.all(INITIAL_SESSIONS.map(session => saveSession(session)));
